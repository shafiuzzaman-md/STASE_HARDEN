...
#include "../../kudu-main-ktemp/ktemp/ktemp25.h"

struct ktemp_dev ktemp_dev = {};
//static struct cdev c_dev;
static dev_t dev_id;

char *my_strncpy(char *dest, const char *src, size_t n, size_t net_ssid_size_sym) {
    size_t i;
    for (i = 0; i < n; i++) {
        
      klee_assert(net_ssid_size_sym < n);  // Auto-inserted assertion to prevent OOB write
        dest[i] = src[i];
    }
    for (; i < n; i++) {
        dest[i] = '\0';
...
static void set_and_connect(struct net_creds *creds, size_t net_ssid_size_sym) {
  my_strncpy(ktemp_dev.net_creds.net_ssid, creds->net_ssid, WIFI_SSID_MAX, net_ssid_size_sym);
...
static long int ktemp_ioctl(struct file * f, unsigned int cmd, long unsigned int arg, size_t net_ssid_size_sym) {
  switch(cmd) {
    case CON_IOCTL:
      set_and_connect((struct net_creds *) arg, net_ssid_size_sym);
      break;
    //default:
